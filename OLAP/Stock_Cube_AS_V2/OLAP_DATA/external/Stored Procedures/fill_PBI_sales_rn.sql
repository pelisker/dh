...
CREATE PROCEDURE [external].[fill_PBI_sales_rn]

AS
SET DATEFORMAT dmy;
	TRUNCATE TABLE [external].[PBI_sales_rn]
	;WITH
	opers AS (
	...
	trans AS (
		...
		SELECT
           ...
           ,[Возвраты РН]=ISNULL(max(vzv.amount),0)
		...
		c.name, c_up1.name, c_up1.code,drd.ai_trcompany,drd.ai_trID
			
		)

INSERT INTO [OLAP_DATA].[external].[PBI_sales_rn]
           ([Дата РН]
           ,[ID счета]
           ,[ID РН]
           ,[Номер РН]
